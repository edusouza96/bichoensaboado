...
$idServicVet = $_GET['servicVet'];
$idServic = explode('|', $idServic);
$idServicVet = explode('|', $idServicVet);
...
for ($i = 0; $i < count($idField); $i++) {
    ...
    if ($idServicVet[$i] > 0) {
        $servicVet = $servicDao->SearchId($idServicVet[$i]);
        $diary->servicVet = $servicVet;
    }else{
		$diary->servicVet = null;
	}
    ...
    if ($diary->package->idPackage == 0) {
        ...
        $price = isset($servic) ? $servic->valuation : 0;
        $priceVet = isset($servicVet) ? $servicVet->valuation : 0;
        $diary->price = $price;
        $diary->priceVet = $priceVet;
        $diary->totalPrice = $price + $priceVet+ $deliveryPrice;
    } else {
