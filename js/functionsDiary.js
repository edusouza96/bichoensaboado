...
function updateDiary(idField){
    ...
    if(msgError == ''){
        ...
        if(selectServicEdit.length > 1){
            ...
            var url = "ajax/update.php?idField=" + idFilds.join('|').replace(/serviceSelect/g, '') + "&dateHour=" + dateHour + "&search=" + search + "&deliveryPrice=" + delivery + "&servic=" + services.join('|'); 
        }else{
            var url = "ajax/update.php?idField=" + idField + "&dateHour=" + dateHour + "&search=" + search + "&deliveryPrice=" + delivery + "&servic=" + servic; 
        }
        ajaxUpdate(url);
    }else{
        alert(msgError);            
...
function listServic(idBreed){
    id = idBreed.split('#');
    var url = "ajax/animalSameOwner.php?id=" + id[1] + "&field=servic"; 
